...
class ASDNPretrainLossLayer(caffe.Layer):
    def setup(self, bottom, top):
        ...
        # parse the layer parameter string, which must be valid YAML
        # search for fast_rcnn_adv_pretrain/train.prototxt
        layer_params = yaml.safe_load(self.param_str)
        self._num_classes = layer_params['num_classes']
        ...
        top[0].reshape(1)   #展平操作将数组重塑为一个形状为(1,)的一维数组
    ...
    def forward(self, bottom, top):
        ...
        mask_pred = bottom[0].data   #conv10_mask
        conv_feat_mask = bottom[1].data  #asdn_pretrain_data
        prop =  bottom[2].data
        labels_pos = bottom[3].data
        rois_pos = bottom[4].data
        data = bottom[5].data 

        N = bottom[0].shape[0]  #等于conv_feat_pos的数量
        N2 = bottom[1].shape[0] #等于conv_feat_pos的数量*16（attempts）
        ...
        lZ  = np.log(1+np.exp(-np.abs(f))) * mask  #如果有ignore_label=None 就为1
        ...
        ###### debug 
        debug_sign = True
        debug_folder = '/home/likaiyang/adversarial-frcnn-master_copyend/image_pretrain/'
...
class ASDNPretrainDataLayer(caffe.Layer):
    ...
    def generate_feature(self, conv_feat):
        ...
        # conv_feat_rep = np.zeros((sample_num * rep_num, channels, pool_len, pool_len))
        # conv_feat_mask = np.zeros((sample_num * rep_num, 1, pool_len, pool_len))

        cnt = 0 

        for i in range(rep_num):

            # j = np.random.choice(np.arange(rep_num))
...
class ASDNLossLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        N = bottom[0].shape[0]
#1 0 1 0 每次经过forward后 _count_iter等于
        self._count_iter = (self._count_iter + 1) % self._iter_size
        ...
        ###### debug 
        debug_sign = True
        debug_folder = '/home/likaiyang/adversarial-frcnn-master_copyend/image_adv/'
        ...
        if debug_sign and os.path.exists(debug_folder):
            ...
            for i in range(N):
                ...
                im = data[imid]
                im = im[:, y1:y2, x1:x2]
                im = im.transpose((1, 2, 0)).copy()   #图像转置，图像的维度从 (height, width, channels) 变为 (channels, height, width)
                im += cfg.PIXEL_MEANS
                # im = im[:, :, (2, 1, 0)]
...
class ASDNDataLayer(caffe.Layer):
    ...
    def generate_mask_rand(self, mask_pred):
        ...
        for i in range(sample_num):
            rp = np.random.permutation(np.arange(mask_pixels))   #16取1
            rp = rp[0: count_drop_neg] #count_drop_neg = 0
    ...
    def thres_mask_rand(self, mask_pred, labels):
        ...
        for i in range(sample_num):
            ...
            if labels[i] == 0:
                ...
                rp = np.random.permutation(np.arange(permute_count))  #20(0~19)取15(0~14)
                rp = rp[0: count_drop]#前15

                now_mask_pred = mask_pred[i]
                now_mask_pred_array = np.reshape(now_mask_pred, mask_pixels)

                sorted_ids = np.argsort(now_mask_pred_array)[::-1]  #将array的元素从小到大排列并取出其索引
                now_ids = sorted_ids[rp]

                now_mask = np.ones(mask_pixels)
                now_mask[now_ids] = 0  # 0 means block
                now_mask = np.reshape(now_mask, (pool_len, pool_len))
    ...
    def forward(self, bottom, top):
        ...
        mask_thres_block = np.tile(mask_thres, [1, self._channels, 1, 1])  #复制多少channels
...
class  SAMlayer(caffe.Layer):
    ...
    def forward(self, bottom , top ):
        ...
        max_channel= np.amax(bottom[0].data, axis=1 , keepdims=True)
        print(max_channel.shape)
        ave_channel = np.mean(bottom[0].data, axis=1, keepdims=True)
        print(ave_channel.shape)
        output = np.concatenate( (max_channel , ave_channel ), axis=1)
        top[0].reshape(*(output.shape))
        top[0].data[...] = output.astype(np.float32, copy=False)
...
class  CLAHElayer(caffe.Layer):
    def clahe_lab(self,bgr,gridsize):
        bgr = bgr.transpose((0,2,3,1))
        print("bgr.sha[e]",bgr.shape)
        print(bgr)
        bgr= np.squeeze(bgr)
        print("bgr.sha[e]",bgr.shape)
    ...
    def forward(self, bottom , top ):
        ...
        top[0].reshape(*(bottom[0].data.shape))
        top[0].data[...] = self.clahe_lab(bottom[0].data,8)
...
# net = caffe.Net('/home/likaiyang/adversarial-frcnn-master_copyend/models_change/pascal_voc/VGG16/faster_rcnn_end2end/CBAM.prototxt',caffe.TEST)
# im = np.array(cv2.imread('/home/likaiyang/adversarial-frcnn-master_copyend/data/demo/000456.jpg'))
# imput = im[np.newaxis, : , : ]
# print('imput:.shpae',imput.shape)
# imput2 = imput.transpose((0,3,1,2))
# print('imput2.shape:',imput2.shape)
# net.blobs['data'].reshape(*imput2.shape)
# net.blobs['data'].data[...] = imput2
# net.forward()
