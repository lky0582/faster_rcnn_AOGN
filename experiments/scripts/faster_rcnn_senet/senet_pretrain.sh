...
LOG="experiments/logs/faster_rcnn_senet_pretrain.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
time ./tools/train_net.py --gpu ${GPU_ID} \
  --solver models_change/${PT_DIR}/${NET}/faster_rcnn_end2end_adv_pretrain/solver_senet.prototxt \
  --weights ../../../data/li/output/faster_rcnn_senet/faster_rcnn_senet_iter_30000.caffemodel \
  --imdb ${TRAIN_IMDB} \
  --iters ${ITERS} \
  --cfg 'experiments/cfgs/fast_rcnn_adv_pretrain.yml' \
  ${EXTRA_ARGS}

# solver models有对应的prototxt
