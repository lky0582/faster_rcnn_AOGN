...
#在训练前要看一看一些路径是否正确，比如模型保存的路径和测试路径是否一一致
LOG="experiments/logs/faster_rcnn_senet_adv.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
echo Logging output to "$LOG"


time ./tools/train_net.py --gpu ${GPU_ID} \
  --solver models_change/${PT_DIR}/${NET}/faster_rcnn_end2end_adv/solver_senet.prototxt \
    --weights output/fast_rcnn_adv/voc_2007_trainval/train_init_senet.caffemodel \
  --imdb ${TRAIN_IMDB} \
  --iters ${ITERS} \
