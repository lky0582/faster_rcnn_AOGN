...
case $DATASET in
  pascal_voc)
    ...
    PT_DIR="pascal_voc"
    ITERS=30000        #original setting=10000
    ;;
  coco)
...
LOG="experiments/logs/faster_rcnn_end2end_senet.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
echo Logging output to "$LOG"

time ./tools/train_net.py --gpu ${GPU_ID} \
  --solver models_change/${PT_DIR}/${NET}/faster_rcnn_end2end/solver_senet.prototxt \
  --weights  data/imagenet_models/SE-ResNet-50.caffemodel \
  --imdb ${TRAIN_IMDB} \
  --iters ${ITERS} \
